...
export function getUnderlyingExposureByBucket(
  ...
  // so keep their statement-anchored stored MV for that bucket only. Other buckets may hold the
  // same ticker and still need live spot/synthetic exposure.
  ...
  for (const r of spot) {
    ...
    if (r.is_plan_fund) planFundBucketSymbols.add(`${bucket}:${symKey}`);
  ...
  for (const [bucket, m] of byBucket.entries()) {
    for (const row of m.values()) {
      ...
      if (px == null) continue;
      const preservePlanSpot = planFundBucketSymbols.has(`${bucket}:${row.underlyingSymbol}`);
      if (!preservePlanSpot && row.heldShares > 0) row.spotMarketValue = row.heldShares * px;
