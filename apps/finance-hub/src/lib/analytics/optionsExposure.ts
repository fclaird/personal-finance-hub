...
export function getUnderlyingExposureByBucket(
  ...
  for (const [bucketKey, m] of byBucket.entries()) {
    for (const row of m.values()) {
      ...
      if (px == null) continue;
      // Plan/529 fund share counts are synthetic; preserve their statement-anchored spot MV.
      if (!planFundBucketSymbols.has(`${bucketKey}:${row.underlyingSymbol}`) && row.heldShares > 0) {
