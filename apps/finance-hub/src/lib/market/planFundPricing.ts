...
/** Re-basis only for legacy anchors that used an obviously fake fallback NAV. */
export function repairFundBasisIfMarkDrift(
  basis: FundStatementBasis,
  navToday: number,
): FundStatementBasis | null {
  const marked = markToMarketFund(basis, navToday);
  const extremeScale = marked > basis.statementMarketValue * 25;
  const fakeFallbackNav = basis.basisTickerNav <= 1 && navToday > 10;
  if (!fakeFallbackNav && !extremeScale) return null;
