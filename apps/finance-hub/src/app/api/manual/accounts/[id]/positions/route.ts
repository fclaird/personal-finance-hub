...
type ManualPositionRequest = Partial<ManualPositionInput> & { anchorStatementBalance?: boolean };
...
export async function POST(req: Request, ctx: RouteCtx) {
  try {
    ...
    const shouldAnchorFund = securityType === "fund" && (!body.positionId || body.anchorStatementBalance === true);
    if (shouldAnchorFund && marketValue != null && Number.isFinite(marketValue) && marketValue > 0 && symbol) {
      ...
      if (body.anchorStatementBalance === true && !fundBasis) {
        return NextResponse.json(
          { ok: false, error: "Unable to anchor fund statement balance because the public NAV is unavailable." },
          { status: 400 },
        );
      }
    }
