...
import { loadRecentNewsItems } from "@/lib/news/store";
import { mergeDedupedNews } from "@/lib/news/mergeDeduped";
import { textMatchesSymbol } from "@/lib/news/matchSymbols";
...
function filterIngestForCompany(items: NewsItem[], sym: string, companyName: string | null): NewsItem[] {
  const s = normSym(sym);
  if (!s) return [];
  return items.filter((it) => {
    if (it.symbols.some((x) => normSym(x) === s)) return true;
    const text = `${it.title}\n${it.body ?? ""}`;
    return textMatchesSymbol(text, sym, companyName);
  });
}
...
export async function GET(req: Request) {
  ...
  const sourcesParam = (url.searchParams.get("sources") ?? "all").toLowerCase();
  const includeIngest = sourcesParam === "all" || sourcesParam === "ingest";
  const includeRss = sourcesParam === "all" || sourcesParam === "rss";
  ...
  const db = getDb();
  const ingestItems = includeIngest ? loadRecentNewsItems(db, { limit: 60 }) : [];

  const macroFeeds = [...DEFAULT_MACRO_FEEDS, ...extraFeedsFromEnv()];

  if (mode === "company") {
    const sym = focus[0] ?? "";
    const ingestFiltered = sym ? filterIngestForCompany(ingestItems, sym, companyName) : [];
    const yahoo = includeRss && sym ? await fetchSymbolNews(sym, "company") : [];
    const globalFeeds = includeRss
      ? await Promise.allSettled(macroFeeds.map((f) => fetchMacroFeed(f)))
      : [];
    const globalItems = globalFeeds.flatMap((r) => (r.status === "fulfilled" ? r.value : []));
    const filtered = sym
      ? globalItems.filter((it) => textMatchesSymbol(it.title, sym, companyName))
      : [];
    const merged = mergeDedupedNews([...ingestFiltered, ...yahoo, ...filtered]).slice(0, MAX_RESPONSE_ITEMS);
    return NextResponse.json({ ok: true, mode: "company", sources: sourcesParam, items: merged });
  }

  const yahooSets = includeRss
    ? await Promise.all([
  ...
  const globalItems = globalFeeds.flatMap((r) => (r.status === "fulfilled" ? r.value : []));
  const merged = mergeDedupedNews([...ingestItems, ...yahooItems, ...globalItems]).slice(0, MAX_RESPONSE_ITEMS);

  return NextResponse.json({ ok: true, mode: mode || "default", sources: sourcesParam, items: merged });
}
