...
export default function TerminalSymbolPage() {
  ...
  async function loadAll() {
    setError(null);
    try {
      const storyCacheUrl = `/api/symbols/story?symbol=${encodeURIComponent(sym)}&mode=cache`;
      ...
      void (async () => {
        try {
          ...
          const revResp = await fetch(
            `/api/symbols/story?symbol=${encodeURIComponent(sym)}&mode=revalidate`,
            { cache: "no-store" },
          );
      ...
      <DraggableTileLayout
        ...
        <div className="mt-4 rounded-xl border border-zinc-300 bg-white/60 p-4 dark:border-white/20 dark:bg-black/20">
          <NewsFeedPanel
            ...
            mode="company"
            symbols={[sym]}
            companyName={company?.ok === true ? company.companyName : null}
            maxItems={16}
          />
        </div>
